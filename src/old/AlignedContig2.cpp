...
// variant validator min size to count discordants
#define MIN_SIZE_FOR_DISC 1000
...
namespace SnowTools2 {
  ...
  AlignedContig::AlignedContig(const BamReadVector& bav, const std::set<std::string>& prf) {
  ...
  void AlignedContig::printContigFasta(std::ofstream& os) const {
    os << ">" << getContigScrubName() << std::endl;
  ...
  std::ostream& operator<<(std::ostream& out, const AlignedContig &ac) {
	...
	ac.getContigScrubName() << std::endl;       
	...
	out << j << " -- " << ac.getContigScrubName() << std::endl;
	//out << j << " Disc: " << ac.printDiscordantClusters() << " -- " << ac.getContigScrubName() << std::endl;
      }
	...
	if (j.num_align == 1 && j.insertion == "") // deletion
	  //std::cerr << j.b1.cpos << " " << j.b2.cpos << " name " << ac.getContigScrubName() << " ins " << j.insertion << std::endl;
	  out << std::string(j.b1.cpos, ' ') << "|" << std::string(j.b2.cpos-j.b1.cpos-1, ' ') << '|' << "   " << ac.getContigScrubName() << std::endl;	
      }
    }
    ...
    out << ac.getSequence() << "    " << ac.getContigScrubName() << std::endl; 
    ...
    for (auto& i : ac.m_bamreads) {
      ...
      std::vector<int> asvec = i.GetSmartIntTag("AA"); // alignment score
	...
	try { 
      ...
      assert(pos < MAX_CONTIG_SIZE && padlen < MAX_CONTIG_SIZE); // bug, need to check
		     rstream << sr << "--" << (i.ChrID()+1) << ":" << i.Position() << " r2c CIGAR: " << this_cig << " CPOS:" << pos << " R2CAS:" << as;
      
      // if decoyed, make small
      if (i.GetIntTag("DD")) {
  ...
  */
  ...
  int AlignedContig::countDiscordants() {

    // only do if size big enough
    if (m_global_bp.getSpan() < MIN_SIZE_FOR_DISC && m_global_bp.getSpan() > 0)
      return 0;
  ...
  std::unordered_map<std::string, std::string> read_to_bam; // first - qname, second -- sample id

  for (auto& r : m_bamreads) {

    // don't count discordants
    if (r.GetIntTag("DD"))
      continue;
    ...
    //if (r.Qname() == "D0UW5ACXX120511:8:2305:16484:66796")
    //  std::cerr << r.GetZTag("SR") << " pos " << pos << " cpos " << cpos1 << " cpos2 " << cpos2 << " flag " << flag << " all[qq].first " << all[qq].first << " " << all[qq].second << std::endl;

    // store a mapping of qnames back to prefixes (e.g. t000)
    read_to_bam[qq] = r.GetZTag("SR").substr(0,4);
    
  }
  
  discordants = 0;

  std::unordered_set<std::string> to_add_back;
  for(auto& i : all) {
    if (i.second.first != i.second.second && i.second.first + i.second.second == 3) {
      ++discordants;
      std::string t000 = read_to_bam[i.first];      // i.first = qname
      m_global_bp.allele[t000].supporting_reads.insert(i.first);
      ++m_global_bp.allele[t000].disc;
      to_add_back.insert(i.first);
    }
  }

  // put the discordant reads into BP reads (for printing later)
  for (auto& r : m_bamreads) {
    if (to_add_back.count(r.Qname()))
      m_global_bp.reads.push_back(r); // ok if they dupe with split, since all stats are on SampleInfo reads, and print already dedupes
  }
  ...
  void AlignedContig::decoyReads(const bam_hdr_t *h, SnowTools::RefGenome& ref) {
    
    // double check the discordants against opposite reference
    SnowTools::BWAWrapper side1;
    SnowTools::BWAWrapper side2;
    
    const GenomicRegion * gr1 = &(m_global_bp.b1.gr);
    const GenomicRegion * gr2 = &(m_global_bp.b2.gr);

    int width = 500;

    // can't decoy if very small
    if (std::abs(gr1->pos1 - gr2->pos1) < width && gr1->chr == gr2->chr)
      return;
    
    // marcin convention. want to grab opposite side though as decoy, but bp
    // is jeremiah convention.... so keep same as in SVVCF
    int p1 = gr1->strand == '+' ? gr1->pos1 : gr1->pos1 - width;
    ...
    SnowTools::USeqVector usv1, usv2;
    usv1.push_back({std::string("side1"), seq1});
    side1.constructIndex(usv1);
    usv2.push_back({std::string("side2"), seq2});
    side2.constructIndex(usv2);
    
    SnowTools::BamReadVector m_bamreads_new;

    for (auto& r : m_bamreads) {
      
      SnowTools::BamReadVector brv1, brv2;
      std::string seqr = r.QualitySequence();
      side1.alignSingleSequence(seqr, r.Qname(), brv1, true, 0.90, 1);
      side2.alignSingleSequence(seqr, r.Qname(), brv2, true, 0.90, 1);
      
      bool ok = true;
      if (brv1.size()) {
	bool length_pass = (brv1[0].PositionEnd() - brv1[0].Position()) >= ((double)seqr.length() * 0.75);    
	bool clip_pass = brv1[0].NumClip() < 0.5 * (double)seqr.length();
	bool score_pass = brv1[0].GetIntTag("AS") > (double)seqr.length() * 0.5;
	if (length_pass && clip_pass && score_pass) {
	  ok = false;
	  ++m_global_bp.allele[r.GetZTag("SR").substr(0,4)].decoyed.first;
	  //std::cerr<< "...aligned to decoy1: " << r << " " << brv1[0] << std::endl << "..." << *gr1 << " -- " << *gr2 << std::endl;
	}

      }
      if (brv2.size()) {
	bool length_pass = (brv2[0].PositionEnd() - brv2[0].Position()) >= ((double)seqr.length() * 0.75);    
	bool clip_pass = brv2[0].NumClip() < 0.5 * (double)seqr.length();
	bool score_pass = brv2[0].GetIntTag("AS") > (double)seqr.length() * 0.5;
	if (length_pass && clip_pass && score_pass) {
	  ok = false;
	  ++m_global_bp.allele[r.GetZTag("SR").substr(0,4)].decoyed.second;
	  //std::cerr<< "...aligned to decoy2: " << r << " " << brv2[0] << std::endl << "..." << *gr1 << " -- " << *gr2 << std::endl;
	}
      }
      
      if (ok) {
	m_bamreads_new.push_back(r);
      } else {
	++m_global_bp.allele[r.GetZTag("SR").substr(0,4)].decoy;
	r.AddIntTag("DD", 1);
	m_bamreads_new.push_back(r);
      }
    ...
    m_bamreads = m_bamreads_new;
    
  }

  void AlignedContig::forceBreakPoint(const SnowTools::GenomicRegion& gr1, const SnowTools::GenomicRegion& gr2) {
    
    // force breakpoint
    if (gr1 < gr2) {
      m_global_bp.b1.gr = gr1;
      m_global_bp.b2.gr = gr2;
    }
    else {
      m_global_bp.b2.gr = gr1;
      m_global_bp.b1.gr = gr2;
    }
      
    // input is from marcin convention, m_global_bp is jeremiah convention. sync it
    m_global_bp.b1.gr.strand = m_global_bp.b1.gr.strand == '+' ? '-' : '+';
    m_global_bp.b2.gr.strand = m_global_bp.b2.gr.strand == '+' ? '-' : '+';

  }
  
  void AlignedContig::scrubName() {
    m_global_bp.scrubName();
  }
