...
namespace SnowTools2 {
  ...
  void BreakPoint::scrubName() {
    cname = SnowTools::scrubString(cname, "_INSRC_1");
    cname = SnowTools::scrubString(cname, "_INSRC_0");
  }

  std::string BreakPoint::toValidatorFileString(bool noreads) {
    ...
    ss << cname << sep << b1.chr_name << sep << b1.gr.pos1 << sep << b1.gr.strand << sep 
    ...
    for (auto& a : allele)
      ss << sep << a.second.toValidatorFileString();
  ...
  */
  ...
  void BreakPoint::splitCoverage(BamReadVector &bav) {
    ...
    for (auto& j : bav) {
      ...
      if (j.GetIntTag("DD")) // dont score decoyed reads
	continue;
	...
	for (; kk < cnvec.size(); kk++)  // find the right contig r2c cigar (since can have one read to many contig)
	  if (cnvec[kk] == cname)  { // found the right cigar string
	    ...
	    SnowTools::Cigar tcig = SnowTools::cigarFromString(cigvec[kk]);
	...
	allele[sample_id].supporting_reads.insert(qn);
  ...
  */
  ...
  void BreakPoint::scoreBreakpoint(double LOD_CUTOFF, double DBCUTOFF, double NODBCUTOFF, double LRCUTOFF) {
	...
	if (r.GetZTag("SR").substr(0,4) != i.first)
	  continue;
	...
	if (!r.MappedFlag() && !r.MateMappedFlag())
	  ++i.second.uumap;
	SnowTools::GenomicRegion gg = r.asGenomicRegion();
	int dd1  = gg.distanceBetweenStarts(b1.gr);
	int dd2  = gg.distanceBetweenStarts(b2.gr);
	if ( (dd1 == -1 || dd1 > 1500) && ( dd2 == -1 || dd2 > 1500 ) )
	  ++i.second.non_local;
      }
      
    }
    
    return; // for VariantValidator, just finish here
  ...
  std::string BreakPoint::__format_readname_string() {
    ...
    for (auto& i : supp_reads) {
      if (++lim > READ_PRINT_LIM)
	break;

      boost::regex regr(".*?_[0-9]+_(.*)"); 
      boost::cmatch pmatch;
      if (/*i.first.at(0) == 't' && */boost::regex_match(i.first.c_str(), pmatch, regr)) 
	supporting_reads = supporting_reads + "," + i.first.substr(0,4) + "_" + pmatch[1].str();
    }
  ...
  std::string SampleInfo::toValidatorFileString() const {

    std::stringstream ss;
    
    ss << std::setprecision(4) << alt << ":" << split
       << ":" << disc << ":" << decoy << ":" << umap << ":" << uumap << ":" << non_local;;
